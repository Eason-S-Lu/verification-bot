...
async def send_verification_code(ctx):
    ...
    def check_email(message):
        return message.author == ctx.author and '@' in message.content and isinstance(message.channel, discord.TextChannel)
        ...
        try:
            ...
            if send_result is not None:
                ...
                def check_verification_code(message):
                    return message.author == ctx.author and message.content == verification_code and isinstance(message.channel, discord.TextChannel)

                try:
                    message = await bot.wait_for('message', timeout=60.0, check=check_verification_code)
                except asyncio.TimeoutError:
                    await ctx.send('Verification timed out. Please try again.')
                else:
                    guild = ctx.guild
...
async def on_member_join(member):
    user = member
    await user.send(f'Welcome to the server, {user.mention}! Please verify yourself by using the `~verify` command.')
